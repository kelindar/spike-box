namespace Spike.Scripting.Native
...
module Utils = 
  ...
  ///
  let private createHostFunctionObject (env:Env) (length:int option) (func:'a) metaData =
    let length =
      match length with
      | Some length -> length
      | None -> 
  ...
  ///
  let createConstructor (env:Env) (length:int option) (ctor:'a) =
    let compiler = Compiler.HostFunction.compile<'a>
    let metaData = env.CreateHostConstructorMetaData(FunctionCompiler(compiler))
    createHostFunctionObject env length ctor metaData

  let CreateConstructor (env:Env) (length:Nullable<int>) (func:'a) =
    let length =
      if length.HasValue
        then Some(length.Value)
        else None
  ...
  let internal trapSyntaxError (env:Env) (f:unit -> 'a) =
      ...
      with
      | :? Spike.Scripting.Runtime.UserError as x ->
        raise x
