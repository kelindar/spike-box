namespace Spike.Scripting.Compiler

open System
open Spike.Scripting
open Spike.Scripting.Dlr
open Spike.Scripting.Dlr.Operators
open Spike.Scripting.Dlr.ExtensionMethods
open Spike.Scripting.Runtime
open Spike.Scripting.Support.CustomOperators
open Spike.Scripting.Compiler
open Spike.Scripting.Compiler.Utils
open Spike.Scripting.Compiler.Context
...
module internal Object =
  ...
  module Index =
    ...
    let private putConvert expr index value tag =
      Utils.tempBlockT<ScriptObject> expr (fun tmp -> 
  ...
  let putMember (ctx:Ctx) (expr:Dlr.Expr) (name:string) (value:Dlr.Expr) =
    ...
    //
    let fromJsFunction (jsobj:Dlr.Expr) (name:string) (value:Dlr.Expr) (ctx:Ctx) =
      let env = ctx.Target.Environment
      let cache, cacheId, cacheIndex = env |> makePropertyPutCache
      let prop = Dlr.const' name
    ...
    let fromJsObject (jsobj:Dlr.Expr) (name:string) (value:Dlr.Expr) (ctx:Ctx) =
      ...
      | _ ->
        ...
        Dlr.Fast.block [||] [|
          (Dlr.call jsobj "OnChange" [prop; value])
          Dlr.ifElse 
            (cacheId .== jsobj .-> "PropertySchema" .-> "Id")
            (Dlr.Fast.block [|index; properties|] [|
              properties .= jsobj .-> "Properties"
    ...
    let fromBox (expr:Dlr.Expr) (name:string) throw (ctx:Ctx) =
      Dlr.ternary
        (Utils.Box.isFunction expr)
        (fromJsFunction (Utils.Box.unboxFunction expr) name throw ctx)
        (Dlr.ternary
    ...
    | TypeTags.Function ->
      body.Add(fromJsFunction expr name value ctx)
