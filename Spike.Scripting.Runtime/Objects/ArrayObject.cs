...
namespace Spike.Scripting.Runtime
{
    ...
    public sealed partial class ArrayObject : ScriptObject
    {
        ...
        private void PutLength(uint newLength)
        {
            ...
            int index = 0;
            if (this.CanPut("length", out index))
            {
                this.Properties[index].Value.Number = (double)newLength;
        ...
        public override void Put(uint index, BoxedValue value)
        {
            ...
            if (this.isDense)
            {
                ...
                {
                    // Make sure we invoke the property changed handler with the index instead
                    // of a name, allowing observation of arrays.
                    if (this.IsObserved)
                    {
                        // Put can be used for both, set and put, distinguish two cases here
                        if (this.dense[index].HasValue)
                        {
                            // Invoke the set
                            this.OnPropertyChange(PropertyChangeType.Set, index.ToString(), value, this.dense[index].Value);
                        }
                        else
                        {
                            // Invoke the put
                            this.OnPropertyChange(PropertyChangeType.Put, index.ToString(), value, Undefined.Boxed);
                        }
                    }
            ...
            // of a name, allowing observation of arrays.
            if (this.IsObserved)
                this.OnPropertyChange(PropertyChangeType.Put, index.ToString(), value, sparse.Get(index));

            // Put in a sparse array
        ...
        public override bool Delete(uint index)
        {
            ...
            if (this.HasIndex(index))
            {
                if (this.isDense)
                {
                    ...
                    // of a name, allowing observation of arrays.
                    if (this.IsObserved)
                        this.OnPropertyChange(PropertyChangeType.Delete, index.ToString(), Undefined.Boxed, this.dense[index].Value);
                ...
                else
                {
                    ...
                    // of a name, allowing observation of arrays.
                    if (this.IsObserved)
                        this.OnPropertyChange(PropertyChangeType.Delete, index.ToString(), Undefined.Boxed, this.sparse.Get(index));

                    // Remove from the sparse array.
        ...
        public bool QuickDelete(string name)
        {
            uint index;
            if (uint.TryParse(name, out index))
                return this.QuickDelete(index);

            return base.Delete(name);
        }


        public bool QuickDelete(uint index)
        {
            if (index == uint.MaxValue)
            {
                return base.Delete(index.ToString());
